...
import { HeartIcon, RedHeartIcon } from 'ui'
...
function ListingImages({
  ...
  let style =
    data.length < 3
      ? `grid-cols-${data.length} grid-rows-${data.length}`
      : 'sm:grid-cols-2 sm:grid-rows-3 grid-rows-2 grid-cols-3'
  return (
    <div className="w-full h-full bg-slate-100 sm:w-[36%] min-w-[250px] relative">
      <div className={`grid w-full h-full gap-[1px] ${style}`}>
        {data.map((image, idx) => {
          const lastIndex = idx === data.length - 1
          const firstImage =
            data.length < 3
              ? 'sm:col-span-2 sm:row-span-1 row-span-2'
              : idx == 0
              ? 'sm:row-span-2 sm:col-span-2 row-span-3 col-span-2'
        ...
        {!isNested && (
          <div className="w-6 h-6 absolute bottom-2 left-3 sm:top-3 sm:right-3 sm:left-[unset] sm:bottom-[unset] hover:cursor-pointer hover:scale-110 transition-transform duration-300">
