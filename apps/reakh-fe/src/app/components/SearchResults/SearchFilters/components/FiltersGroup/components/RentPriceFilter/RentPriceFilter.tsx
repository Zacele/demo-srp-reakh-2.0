...
const RentPriceFilter: React.FC<{
  ...
  const priceMinValue = watch('rent_min__gte')
  const priceMaxValue = watch('rent_min__lte')
  const priceMinSearchFilters = searchParams.get('rent_min__gte')
  const priceMaxSearchFilters = searchParams.get('rent_min__lte')

  React.useEffect(() => {
    register('rent_min__gte')
    register('rent_min__lte')
  }, [register])

  React.useEffect(() => {
    // Update rent_min__gte and rent_min__lte values when priceMinValue or priceMaxValue changes
    if (priceMinValue && priceMaxValue) {
      setValue('rent_min__gte', Math.min(Number(priceMinValue), Number(priceMaxValue)))
      setValue('rent_min__lte', Math.max(Number(priceMinValue), Number(priceMaxValue)))
      handleSubmit(onSubmit)()
    }

    // Update priceMinValue value when priceMinValue
    if (priceMinValue && !priceMaxValue) {
      setValue('rent_min__gte', priceMinValue)
      handleSubmit(onSubmit)()
    }

    // Update priceMaxValue value when priceMaxValue
    if (priceMaxValue && !priceMinValue) {
      setValue('rent_min__lte', priceMaxValue)
      handleSubmit(onSubmit)()
    }
    ...
    if (!priceMinValue && !priceMaxValue && (priceMinSearchFilters || priceMaxSearchFilters)) {
      setValue('rent_min__gte', '')
      setValue('rent_min__lte', '')
      handleSubmit(onSubmit)()
    }
  ...
  React.useEffect(() => {
    if (!priceMinSearchFilters) setValue('rent_min__gte', '')
    if (!priceMaxSearchFilters) setValue('rent_min__lte', '')
